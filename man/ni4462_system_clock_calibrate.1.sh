...
#Program name.
NAME="ni4462_system_clock_calibrate"

#The binary, (relative path to this script). Invoked with "-h" for help text (stdout or stderr)
BINARY=../src/ni4462_system_clock_calibrate.sh

#Description: brief string for the start of the man page.
DESCRIPTION="calibrate the NI4462' clock against a reference."

#Synopsis text, or leave blank to omit. Add leading spaces to avoid automatic paragraph formatting.
SYNOPSIS=`cat <<-EOT
 This checks the ircam's master clock (derived from NI4462, and exported) against an external reference.
 It generates "(internal) 1MHz" pulses and counts them for "(external reference) 10 seconds"
EOT`
...
SEE_ALSO=( "ni4462_test (1)" "pb_freq_gen (1)" "arduino_delay (1)" "filterctl (1)" )
