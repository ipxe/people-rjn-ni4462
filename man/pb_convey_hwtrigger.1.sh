...
#Program name.
NAME="pb_convey_hwtrigger"

#The binary, (relative path to this script). Invoked with "-h" for help text (stdout or stderr)
BINARY=../src/pb_convey_hwtrigger.sh

#Description: brief string for the start of the man page.
DESCRIPTION="constantly (or burst) transfer HW_trigger edges to the PulseBlaster"

#Synopsis text, or leave blank to omit. Add leading spaces to avoid automatic paragraph formatting.
SYNOPSIS=`cat <<-EOT
 Make the NI4462's RTSI6 oscillate at 204 kHz.
 This allows the (modified) PulseBlaster to respond to HW_Trigger within 5us.
EOT`
...
SEE_ALSO=( "pb_start (1)" "pb_serial_trigger (1)" "ni4462_test (1)" )
