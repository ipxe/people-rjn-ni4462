...
#Program name.
NAME="pb_ni4462_trigger"

#The binary, (relative path to this script). Invoked with "-h" for help text (stdout or stderr)
BINARY=../src/pb_ni4462_trigger.sh

#Description: brief string for the start of the man page.
DESCRIPTION="program the pulseblaster to trigger the ni4462."

#Synopsis text, or leave blank to omit. Add leading spaces to avoid automatic paragraph formatting.
SYNOPSIS=`cat <<-EOT
 This programs the PulseBlaster to trigger the NI4462 (on pb_start).
 The NI4462 can be triggered with a single falling/rising edge, continuously at interval T, or N times.
EOT`
