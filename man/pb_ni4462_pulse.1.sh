...
#Program name.
NAME="pb_ni4462_pulse"

#The binary, (relative path to this script). Invoked with "-h" for help text (stdout or stderr)
BINARY=../src/pb_ni4462_pulse.sh

#Description: brief string for the start of the man page.
DESCRIPTION="program the pulseblaster to trigger the ni4462, followed by a delayed, variable-length pulse"

#Synopsis text, or leave blank to omit. Add leading spaces to avoid automatic paragraph formatting.
SYNOPSIS=`cat <<-EOT
 The PulseBlaster is connected so that one of its outputs is wired up to the NI 4462's trigger input, and another
 is looped back to the Analog input (filterctl -k). So it's possible to measure and characterise the trigger-delay
 and the input response shape of the ADC.
EOT`
...
SEE_ALSO=( "ni4462_characterise (1)" "ni4462_test (1)" "arduino_delay (1)" "filterctl (1)" "pb_ni4462_trigger (1)" )
